...
var environment = config['test_0310'].env;
...
console.log('&test_0310 api: [' +  domains[environment] + ', ' + config['test_0310'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['test_0310'].app, config['test_0310'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['test_0310'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
